...
namespace Scada.Server.Svc
{
    ...
    sealed class Comm
    {
        ...
        /// <summary>
        /// Найти в Active Directory группы безопасности, в которые входит заданная группа, и добавить их в список
        /// </summary>
        ...
        private bool CheckUser(string userName, string password, out int roleID)
        {
            ...
            // проверка пользователя по Active Directory
            if (settings.UseAD)
            {
                DirectoryEntry entry = null; // элемент Active Directory
                ...
                try
                {
                    ...
                    if (pwdOk)
                    {
                        // получение групп безопасности пользователя
                        DirectorySearcher search = new DirectorySearcher(entry);
            ...
            if (!pwdOk || roleID == BaseValues.Roles.Err)
            {
                if (pwdOk)
                    roleID = BaseValues.Roles.Disabled;

                MainLogic.User user = mainLogic.GetUser(userName);
                if (user != null && (pwdOk || string.IsNullOrEmpty(password) || password == user.Password))
                {
                    roleID = user.RoleID;
                    pwdOk = true;
                }
